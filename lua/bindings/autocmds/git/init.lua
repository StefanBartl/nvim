---@module 'bindings.autocmds.git'
--- Orchestrates all Git-related autocommands by delegating to submodules.
--- Public entrypoint: require('bindings.autocmds.git').enable(cfg)
...
local Defaults = require("bindings.autocmds.git.defaults").get_defaults()
...
function M.enable(cfg)
  ...
  require("bindings.autocmds.git.commit_ft").enable(cfg.commit_ft, shared)
  require("bindings.autocmds.git.gitsigns_refresh").enable(cfg.gitsigns_refresh, shared)
  require("bindings.autocmds.git.blame_on_hold").enable(cfg.blame_on_hold, shared)
end
