...
local function _dedupe_consecutive(parts)
  ...
  for _, s in ipairs(parts) do
    if s ~= "" and s ~= last then out[#out+1] = s; last = s end
  end
  return out
end
  ...
  for _ = 1, 20 do
    ...
    if t == "pair" then
      ...
      if key then
        ...
        if k ~= "" then table.insert(keys, 1, k); last_key = last_key or k end
      end
...
--- Erzeuge den Kontext-String gemäß der Provider-Reihenfolge und den Toggles.
--- 1) lsp_func → 2) ts_symbol → 3) container (augment) → 4) lang_extra → 5) word
---@return string|nil
local function _build_context()
  local order = (cfg.breadcrumbs_ctx and cfg.breadcrumbs_ctx.providers_order)
                or { "lsp_func", "ts_symbol", "container", "lang_extra", "word" }

  local have ---@type string|nil
  for _, who in ipairs(order) do
    if who == "lsp_func" then
      have = have or _ctx_lsp_func()
    elseif who == "ts_symbol" then
      have = have or _ctx_ts_symbol()
    elseif who == "container" then
      if have then have = _ctx_with_container(have) end
    elseif who == "lang_extra" then
      have = have or _ctx_lang_extra()
    elseif who == "word" then
      have = have or _ctx_word_fallback()
    end
  end
  return have
end
