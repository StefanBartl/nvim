...
local SKIP_FTS = {
  "neo-tree", "lazygit", "TelescopePrompt", "aerial",
  "help", "qf", "oil", "dap-repl", "toggleterm",
}

local function current_win_is_skippable()
  local win = vim.api.nvim_get_current_win()
  if not vim.api.nvim_win_is_valid(win) then return true end
  local buf = vim.api.nvim_win_get_buf(win)
  if vim.bo[buf].buftype ~= "" then return true end -- terminal/prompt/help/qf/…
  local ft = vim.bo[buf].filetype
  for _, s in ipairs(SKIP_FTS) do
    if ft == s then return true end
  end
  return false
end
...
local function set_active_window_line_tint(mode)
  ...
  if last_mode_by_win[win] == m then return end
  ...
  if wh == vim.wo.winhighlight
     and vim.wo.cursorline == cfg.enable_line
     and vim.wo.cursorlineopt == "both"
  then
    return
  end
