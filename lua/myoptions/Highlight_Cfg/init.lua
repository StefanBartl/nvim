...
-- Namespaces & groups
local NS_INDENT      = api.nvim_create_namespace("myopt_IndentScope")
local NS_FLASH       = api.nvim_create_namespace("myopt_Flash")
local AUG_COLOR      = api.nvim_create_augroup("myopt_ColorPersist", { clear = true })
local AUG_WIN        = api.nvim_create_augroup("myopt_PerWindow", { clear = true })
local AUG_MODE       = api.nvim_create_augroup("myopt_Mode", { clear = true })
local AUG_FLASH      = api.nvim_create_augroup("myopt_FlashAuto", { clear = true })
local AUG_TERM       = api.nvim_create_augroup("myopt_TermPalette", { clear = true })
local AUG_DIAG       = api.nvim_create_augroup("myopt_DiagTint", { clear = true })
local AUG_CWORD      = api.nvim_create_augroup("myopt_CWordAuto", { clear = true })
local AUG_VIEW       = api.nvim_create_augroup("myopt_ViewUpdates", { clear = true })

--- Convert a hex codepoint string (e.g. "F0056") to a UTF-8 char using Neoapi.
...
local function _cp(hex)
	...
	local ok, ch = pcall(fn.nr2char, n)
...
local function set_active_window_line_tint(mode)
	...
	if wo.cursorcolumn then wh = wh .. ",CursorColumn:CursorColumn" end

	-- keep SignColumn mapping if present
	local existing = wo.winhighlight
...
local function activate_window_hl()
	local m = (fn.mode(1) or "n"):sub(1, 1)
	if should_enable_column() then wo.cursorcolumn = true else wo.cursorcolumn = false end
...
local function deactivate_window_hl()
	...
	local wh = wo.winhighlight or ""
	wh = wh:gsub("CursorLine:[^,%s]+,?", "")
			:gsub("CursorLineNr:[^,%s]+,?", "")
			:gsub("CursorColumn:[^,%s]+,?", "")
			:gsub(",+", ","):gsub("^,", ""):gsub(",$", "")
	wo.winhighlight = (wh ~= "" and (wh .. ",") or "") .. "CursorLine:Normal,CursorLineNr:LineNr,CursorColumn:Normal"
end
...
local function _ts_symbol_path_fallback()
	...
	local function ident_of(n)
		...
		local function first_ident(m, d)
			...
			if want[m:type()] then
				local s = txt(m); if s ~= "" then return s end
			end
...
local function highlight_full_lines(buf, ns, hl, srow, erow, priority)
	...
	local last0 = api.nvim_buf_line_count(buf) - 1
	...
	if vhl and type(vhl.range) == "function" then
		vhl.range(buf, ns, hl, { srow, 0 }, { end_row, end_col }, { inclusive = inclusive, priority = priority })
		return
	end
...
local function update_indent_scope()
	if not cfg.enable_indent_scope or is_large_file_any() then
		api.nvim_buf_clear_namespace(0, NS_INDENT, 0, -1); return
	end
	local bufnr = 0
	if indent_scope_should_skip(bufnr) then
		api.nvim_buf_clear_namespace(bufnr, NS_INDENT, 0, -1); return
	end
	local cur = api.nwin_get_cursor(0)
	local row = cur[1]
	local topl = fn.line("w0")
	local botl = fn.line("w$")
	local lines = api.nvim_buf_get_lines(bufnr, topl - 1, botl, false)
	local ts = bo.tabstop
	...
	local curline = api.nvim_buf_get_lines(bufnr, row - 1, row, false)[1] or ""
...
local function flash_changed_region(group, ms)
	local bufnr = 0
	local srow, scol = unpack(api.nvim_buf_get_mark(bufnr, "["))
	local erow, ecol = unpack(api.nvim_buf_get_mark(bufnr, "]"))
	if srow == 0 or erow == 0 then return end
	srow = srow - 1; erow = erow - 1
	api.nvim_buf_clear_namespace(bufnr, NS_FLASH, 0, -1)
	local function hl_range(buf, ns, hl, sr, sc, er, ec, prio)
		local ok = (vhl and type(vhl.range) == "function")
		if ok then
			vhl.range(buf, ns, hl, { sr, sc }, { er, ec }, { inclusive = true, priority = prio or 90 }); return
		end
		...
		api.nvim_buf_set_extmark(buf, ns, sr, sc, { end_row = er, end_col = ec, hl_group = hl, priority = prio or 90 })
	end
...
local function ensure_yank_put_flash()
	api.nvim_clear_autocmds({ group = AUG_FLASH })
	...
	if cfg.enable_put_flash and cfg.map_put_flash then
		local function paste_and_flash(which)
			return function()
				api.nvim_feedkeys(which, "n", false)
...
local function apply_signcolumn_tint()
	...
	if #diags == 0 then
		wo.winhighlight = wh .. "SignColumn:SignColNeutral"; return
	end
	local worst
	...
	local grp = (worst == sev.ERROR and "SignColError")
			...
			or "SignColNeutral"
	wo.winhighlight = wh .. "SignColumn:" .. grp
end

---@return nil
local function ensure_diag_tint_autocmd()
	api.nvim_clear_autocmds({ group = AUG_DIAG })
...
local function ensure_term_palette_autocmd()
	api.nvim_clear_autocmds({ group = AUG_TERM })
	if cfg.enable_terminal_palette then
		api.nvim_create_autocmd("TermOpen", {
			group = AUG_TERM,
			callback = function()
				wo.winhighlight = "Normal:TermNormal,CursorLine:TermCursorLine,CursorLineNr:CursorLineNr,LineNr:LineNr"
				wo.cursorline = true
...
local function ensure_current_word_autocmd()
	...
	api.nvim_clear_autocmds({ group = AUG_CWORD })

	--- Clear the previous window-local match, if any.
	---@return nil
	local function clear_match()
		-- match ids are window-local; store on w
		if w._myopt_cword_id then
			pcall(fn.matchdelete, w._myopt_cword_id)
			w._myopt_cword_id = nil
		end
	end
	...
	if cfg.enable_current_word then
		...
		local function update()
			...
			if fn.mode():find("i") then return end
			...
			while true do
				...
				local _, s, e = unpack(fn.matchstrpos(line, pat, start))
			...
			if fn.hlexists(hl) == 0 then
				api.nvim_set_hl(0, hl, { underline = true })
			end

			-- add a single-position, window-local match
			w._myopt_cword_id = fn.matchaddpos(hl, { { lnum, col1, len } }, 10) -- cfg: cfg.priority or 10  (10 is priority)
		end
		...
		local _ = pcall(function()
			if w._myopt_cword_id then
				fn.matchdelete(w._myopt_cword_id)
				w._myopt_cword_id = nil
			end
		end)
	end
...
---   3) fallback to current mode via fn.mode(1)
...
local function ensure_mode_changed_autocmd()
	api.nvim_clear_autocmds({ group = AUG_MODE })
...
local function ensure_color_persist_autocmd()
	api.nvim_clear_autocmds({ group = AUG_COLOR })
...
local function ensure_view_updates()
	api.nvim_clear_autocmds({ group = AUG_VIEW })
	api.nvim_create_autocmd({ "BufEnter", "CursorMoved", "WinScrolled" }, {
		group = AUG_VIEW,
		callback = function()
			vim.schedule(function()
...
local function enable()
	...
	api.nvim_clear_autocmds({ group = AUG_WIN })
	...
	api.nvim_create_autocmd({ "BufReadPost", "TextChanged", "TextChangedI" }, {
		...
		callback = function() if wo.cursorline then activate_window_hl() end end,
	})
