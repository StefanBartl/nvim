...
local function in_list(lst, val)
  if not lst or not val or val == "" then
    return false
  end
...
local function set_active_window_line_tint(mode)
  ...
  local hl = cfg.enable_insert_submode_colors and (map[m] or "CursorLineN") or "CursorLine"

  -- Build new winhighlight while preserving unrelated mappings
  ...
  -- If nothing would change, bail out early
  if wh == vim.wo.winhighlight and vim.wo.cursorline == cfg.enable_line and vim.wo.cursorlineopt == "both" then
    return
  end

  -- Schedule the write to avoid mid-event flicker during heavy transitions
  vim.schedule(function()
    if vim.api.nvim_win_is_valid(win) then
      vim.wo[win].winhighlight = wh
      vim.wo[win].cursorline = cfg.enable_line
...
local function winbar_should_skip(bufnr)
  ...
  if rules.skip_floating and wc and wc.relative and wc.relative ~= "" then
    return true
  end
...
local function _ts_symbol_path_fallback()
  ...
  local function ident_of(n)
    ...
    local want =
      { identifier = true, property_identifier = true, field_identifier = true, type_identifier = true, name = true }
...
local function flash_changed_region(group, ms)
  ...
  local function hl_range(buf, ns, hl, sr, sc, er, ec, prio)
    ...
    if ok then
      vim.hl.range(buf, ns, hl, { sr, sc }, { er, ec }, { inclusive = true, priority = prio or 90 })
      return
    end
    local ok2 = (vim.highlight and type(vim.highlight.range) == "function")
    if ok2 then
      vim.highlight.range(buf, ns, hl, { sr, sc }, { er, ec }, { inclusive = true, priority = prio or 90 })
      return
    end
...
local function apply_signcolumn_tint()
  if not cfg.enable_signcolumn_tint then
    return
  end
  if buffer_is_ui_like() then
    return
...
local function ensure_current_word_autocmd()
  ...
  if cfg.enable_current_word then
    ...
    local function update()
      ...
      -- Skip UI-like windows (neo-tree, prompts, terminals, etc.)
      if buffer_is_ui_like() then
        return
      end
...
local function ensure_mode_changed_autocmd()
  ...
  if cfg.enable_insert_submode_colors then
    ...
    -- Keep tint consistent when entering a window
    ...
    -- Clean cache on window close
    vim.api.nvim_create_autocmd("WinClosed", {
      group = AUG_MODE,
      callback = function(ev)
        local id = tonumber(ev.match) or -1
        last_mode_by_win[id] = nil
      end,
      desc = "Purge mode-tint cache",
    })

    -- Initial apply (no-op if skippable)
    set_active_window_line_tint(_resolve_new_mode(nil))
  else
    activate_window_hl()
  end
...
local function ensure_diff_peek_keymap()
  ...
  if not cfg.enable_diff_peek then
    return
  end
