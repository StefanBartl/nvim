...
M.cfg = {
	...
	highlight = {
		...
		indent_scope_skip = {
			...
			filetypes = { -- UIs/Seitenleisten/Pickers, die keine Blockfärbung erhalten sollen.
				"neo-tree", "neo-tree-popup", "NvimTree", "oil",
				...
				"fugitive", "fugitiveblame", "NeogitStatus", "octo", "git", "gitcommit", "lazygit",
				"dapui_scopes", "dapui_breakpoints", "dapui_stacks", "dapui_watches", "dap-repl", "dapui_console",
				"help", "man", "qf", "checkhealth", "which-key", "spectre_panel", "spectre_replace", "neo-term",
				"minipick", "mini.files",
			},
			name_patterns = { -- Zusätzliche Muster zum Überspringen nach Pfad-/Namen.
				"^oil://", "^term://", "^man://",
				".*[\\/]neo%-tree[\\/].*", ".*[\\/]NvimTree[\\/].*",
				".*[\\/]lazy[\\/].*", ".*[\\/]mason[\\/].*",
			},
...
local function parse_value(s)
	...
	if unq then return unq end
	local lower = trimmed:lower()
	if lower == "true" or lower == "on" or lower == "yes" or lower == "1" then return true end
	if lower == "false" or lower == "off" or lower == "no" or lower == "0" then return false end
...
local function set_by_path(t, path, val, toggle_if_bool)
	...
	if #parts == 0 then return false, "Empty key path" end
	local parent = t
	for i = 1, #parts - 1 do
		local seg = parts[i]
	...
	if oldt == "boolean" then
		if newt ~= "boolean" then
			if toggle_if_bool then
				parent[leaf] = not old; return true
			end
	...
	elseif oldt == "number" then
		if newt ~= "number" then return false, ("Expected number for '%s'"):format(path) end
		parent[leaf] = val
		return true
	elseif oldt == "string" then
		if newt ~= "string" then return false, ("Expected string for '%s'"):format(path) end
		parent[leaf] = val
		return true
...
---@param s string|nil
---@return boolean|number|string
function M.parse(s) return parse_value(s) end
