...
-- Mirrors nvchad.stl.utils' `orders.default`, with "replacer_progress"
-- inserted before "cwd". If NvChad ever changes its own default order,
-- update this list to match (see nvchad/stl/utils.lua).
local order = { "mode", "file", "git", "%=", "lsp_msg", "%=", "diagnostics", "lsp", "replacer_progress", "cwd", "cursor" }

M.ui = {
  statusline = {
    order = order,
    modules = {
      -- Only "replacer_progress" is provided here; every other key in
      -- `order` above resolves to NvChad's own built-in "default" theme
      -- module (mode/file/git/lsp_msg/diagnostics/lsp/cwd/cursor), which
