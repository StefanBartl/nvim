...
pcall(vim.api.nvim_create_user_command, "LuaNotesFiles", function()
  M.fzf_files()
end, { desc = "LuaNotes (fzf-lua): Find files with preview in configured directory" })

pcall(vim.api.nvim_create_user_command, "LuaNotesGrep", function()
  M.fzf_grep()
...
vim.keymap.set("n", "<leader>lnf", M.fzf_files, { desc = "WKD Neovim: fzf files" })
vim.keymap.set("n", "<leader>lng", M.fzf_grep, { desc = "WKD Neovim: fzf grep" })
