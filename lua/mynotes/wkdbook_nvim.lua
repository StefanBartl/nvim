...
local CFG = {
  ...
  dir = vim.fn.expand "~" .. "/MyGithub/WKDBooks/Development/wkdbook-Neovim",
  notify = true,
}
...
pcall(vim.api.nvim_create_user_command, "WkdNvimFzfFiles", function()
  M.fzf_files()
end, { desc = "WKD Neovim (fzf-lua): Find files with preview in configured directory" })

pcall(vim.api.nvim_create_user_command, "WkdNvimFzfGrep", function()
  M.fzf_grep()
end, { desc = "WKD Neovim (fzf-lua): Live grep with preview in configured directory" })

pcall(vim.api.nvim_create_user_command, "WkdNvimTelFiles", function()
  M.tel_files()
end, { desc = "WKD Neovim (Telescope): Find files with preview in configured directory" })

pcall(vim.api.nvim_create_user_command, "WkdNvimTelGrep", function()
  M.tel_grep()
