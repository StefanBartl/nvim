...
local CFG = {
  ...
  dir = vim.fn.expand "~" .. "/MyGithub/WKDBooks/",
  notify = true,
}
...
pcall(vim.api.nvim_create_user_command, "WkdFiles", function()
  M.fzf_files()
end, { desc = "WKD (fzf-lua): Find files with preview in configured directory" })

pcall(vim.api.nvim_create_user_command, "WkdGrep", function()
  M.fzf_grep()
end, { desc = "WKD (fzf-lua): Live grep with preview in configured directory" })
