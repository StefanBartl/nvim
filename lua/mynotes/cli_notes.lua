...
local CFG = {
  ...
  dir = vim.fn.expand(vim.env.REPOS_DIR .. "/Notes/MyNotes/CLI-Notes"),
  notify = true,
}
...
local function note(msg, level)
  if CFG.notify ~= false then
    vim.notify("[CLI-Notes " .. msg, level)
  end
end
...
pcall(vim.api.nvim_create_user_command, "CliNotesFiles", function()
  M.fzf_files()
end, { desc = "CLI-Notes (fzf-lua): Find files with preview in configured directory" })

pcall(vim.api.nvim_create_user_command, "CliNotesGrep", function()
  M.fzf_grep()
end, { desc = "CLI-Notes (fzf-lua): Live grep with preview in configured directory" })
