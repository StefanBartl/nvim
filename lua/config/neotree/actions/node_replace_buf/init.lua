...
return function (state, opts)
  ...
  if not state or not state.tree then
    if logger and logger.warn then logger.warn("node_replace_buf: invalid state") end
    return false
  end

  local node = state.tree:get_node()
  if not node then
    if logger and logger.debug then logger.debug("node_replace_buf: no node under cursor") end
    return false
  end

  if node.type == "directory" or node:has_children() then
    if logger and logger.debug then logger.debug("node_replace_buf: node is directory") end
    return false
  end

  local path = node.path or (type(node.get_id) == "function" and node:get_id())
  if not path or path == "" then
    if logger and logger.debug then logger.debug("node_replace_buf: node has no path") end
    return false
  end
  ...
  if buf_to_close and target_win then
    ...
    if not ok then
      if logger and logger.debug then logger.debug("node_replace_buf: user cancelled save prompt") end
      return false
    end
  ...
  if auto_close then
    ...
    if api.nvim_win_is_valid(neo_win) and neo_win ~= target_win then
      ...
      if logger and logger.info then logger.info("node_replace_buf: auto-closed Neo-tree window") end
    end
