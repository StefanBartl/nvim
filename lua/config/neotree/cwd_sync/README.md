...
Nutzt Project-Root (via `utils.lv_project_root`) statt Buffer-Directory.
...
- Custom Markers (via `utils.lv_project_root` config)
...
```lua
-- In utils.lv_project_root oder ähnlichem Modul
...
### v2.0.0 (2024-01)
- ✨ Added `last_file` tracking (prevents false duplicates)
- ✨ Added `pause_sync(ms)` API
- ✨ Added `sync_scheduled` race-condition guard
- 🐛 Fixed: Sync on same directory but different file
- 🐛 Fixed: Race conditions with multiple triggers
- ⚡ Improved: Better User-Navigation detection
- 📝 Removed: `TabEnter` trigger (performance)
...
require("utils.lv_project_root")        -- Optional: Project-Root detection
```
