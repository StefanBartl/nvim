...
local function resolve_spec(spec, fallback)
	...
	if link then return { link = link } end
	-- hex
...
local function refresh(cfg)
	if not cfg.enable or not neotree_visible_in_tab() then return end
	local ok, manager = pcall(require, "neo-tree.sources.manager")
	if ok then
		pcall(manager.refresh, "filesystem", nil, true)
	else
		pcall(function() vim.cmd("silent! Neotree refresh") end)
	end
end
	...
	return function()
		...
		t:start(ms, 0, function()
			vim.schedule(function()
				if S.timer == t then S.timer = nil end
				pcall(fn)
			end)
