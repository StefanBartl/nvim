---@module 'config.telescope.selected_files.compute'
--- Best-effort resolution of absolute index for a given picker and results-row.
--- This module exposes a single pure function `compute_index_from_picker`.
--- It intentionally mirrors the original logic so external callers can continue
--- to pass the same arguments without API changes.
local M = {}
