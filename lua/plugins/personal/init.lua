...
-- 1. Lokales Repos-Verzeichnis ermitteln
if vim.env.REPOS_DIR and vim.fn.isdirectory(vim.env.REPOS_DIR) == 1 then
  local_repos_path = vim.env.REPOS_DIR
else
  -- Fallback: Search for /repos in the root of common drives (Windows & Unix)
  ...
  if local_repos_path then
    vim.notify(
      string.format(
        "[PLUGINS PERSONAL] 'REPOS_DIR' not set. Use local fallbackk: '%s'",
        local_repos_path
      ),
      vim.log.levels.WARN
    )
    ...
    vim.notify(
      "[PLUGINS PERSONAL] No local repository folder found. Switch to remote for local specs.",
      vim.log.levels.INFO
    )
  end
end

--- Helper function: Returns the local path ONLY if we are not in remote fallback mode.
--- If you explicitly want the remote version for a plugin, simply omit 'dir'!
---@param plugin_name string Der Name des Ordners im Repos-Verzeichnis
---@return string|nil
local function local_dev(plugin_name)
...
return {
...

  {
    ...
    config = function()
      local repos = local_repos_path or ""
