...
return apply_source({
...

  {
    ...
    lazy = false,
    config = function()
      -- Shared config lives in config.filetree so plugins/neotree.lua can pass
      -- the same table to require("filetree").attach(opts, cfg) — that injects
      -- these keymaps into neo-tree's `?` cheatsheet.
