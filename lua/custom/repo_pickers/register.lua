...
function M.usercmds(cfg)
  ...
  vim.api.nvim_create_user_command("RepoFindFilesFzf", function(opts)
    ...
    if dir and dir ~= "" then fzf.files({ cwd = dir }) else
  ...
  vim.api.nvim_create_user_command("RepoGrepFzf", function(opts)
    ...
    if dir and dir ~= "" then fzf.live_grep({ cwd = dir }) else
      local sel = router.mk_selector(cfg, "fzf")
  ...
  vim.api.nvim_create_user_command("RepoFindFilesTelescope", function(opts)
    ...
    if dir and dir ~= "" then tb.find_files({ cwd = dir }) else
  ...
  vim.api.nvim_create_user_command("RepoGrepTelescope", function(opts)
    ...
    if dir and dir ~= "" then tb.live_grep({ cwd = dir }) else
      local sel = router.mk_selector(cfg, "telescope")
  ...
  vim.api.nvim_create_user_command("RepoFilesFzf",       function(opts) vim.api.nvim_cmd({ cmd = "RepoFindFilesFzf",       args = opts.fargs or {} }, {}) end, { desc = "[ALIAS] RepoFindFilesFzf", nargs = "?" })
  vim.api.nvim_create_user_command("RepoFilesTelescope", function(opts) vim.api.nvim_cmd({ cmd = "RepoFindFilesTelescope", args = opts.fargs or {} }, {}) end, { desc = "[ALIAS] RepoFindFilesTelescope", nargs = "?" })
end
