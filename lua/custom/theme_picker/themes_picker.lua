...
--- Add this near the top (helpers) ------------------------------------------

---@param name string
---@return fun()|nil teardown  -- call to remove preloader again
local function ensure_base46_theme_resolvable(name)
  -- 1) If the canonical path exists, nothing to do
  local canonical = vim.api.nvim_get_runtime_file("lua/base46/themes/" .. name .. ".lua", true)
  if #canonical > 0 then
    return nil
  end

  -- 2) Otherwise try your curated path: lua/themes/base46/<name>.lua
  local alt = vim.api.nvim_get_runtime_file("lua/themes/base46/" .. name .. ".lua", true)
  if #alt == 0 then
    -- No alternative file found; cannot help
    return nil
  end
  local file = alt[1]

  -- 3) Inject a package.preload shim so that `require("base46.themes.<name>")` works
  local key = "base46.themes." .. name
  local old_preload = package.preload[key]
  package.preload[key] = function()
    -- The theme files usually return a table; `dofile` runs the chunk and returns its `return` value.
    return dofile(file)
  end

  -- 4) Return teardown to restore previous state
  return function()
    package.preload[key] = old_preload
  end
end
...
local function apply_base46_theme(theme)
  -- Make the theme module resolvable even if it only exists under lua/themes/base46/*.lua
  local teardown = ensure_base46_theme_resolvable(theme)

  -- Set compatibility globals expected by various NvChad/Base46 versions
  vim.g.base46_theme = theme
  vim.g.nvchad_theme = theme

  local ok_mod, base46 = pcall(require, "base46")
  if ok_mod then
    -- Prefer the more specific loader if available
    if type(base46.load_theme) == "function" then
      pcall(base46.load_theme, theme)
    elseif type(base46.load_all) == "function" then
  ...
  -- Improve heuristics for external integrations
  vim.g.colors_name = "base46-" .. theme
  vim.cmd("redraw!")

  -- Clean up the temporary preloader
  if teardown then
    teardown()
  end
  return true
