...
function M.apply(bufnr)
  ...
  if not ft or not ft:match("markdown") then
    return
  end
  ...
  end, with(opts, { desc = "[Custom.Markdown.TableView] Toggle table preview at cursor" }))

  -- Select table
  vim.keymap.set({ "n" }, "<leader>tvs", function()
    vim.cmd("TableViewSelect")
  end, with(opts, { desc = "[Custom.Markdown.TableView] Select and preview table" }))

  -- Open in browser
  vim.keymap.set({ "n" }, "<leader>tvb", function()
    vim.cmd("TableViewOpenBrowser")
  end, with(opts, { desc = "[Custom.Markdown.TableView] Open table in browser" }))

  -- Close view
  vim.keymap.set({ "n" }, "<leader>tvc", function()
    vim.cmd("TableViewClose")
  end, with(opts, { desc = "[Custom.Markdown.TableView] Close TableView" }))
end
