...
local function notify(msg, level)
  ...
  vim.schedule(function() vim.notify(msg, level, { title = "md-codefmt" }) end)
end

local function debug(msg)
  vim.schedule(function() vim.notify(msg, vim.log.levels.DEBUG, { title = "md-codefmt" }) end)
end
...
local function apply_replacements_atomic(bufnr, orig_lines, replacements)
  table.sort(replacements, function(a, b) return a.start_row < b.start_row end)
  local final = {}
  local cur = 1
  for _, r in ipairs(replacements) do
    ...
    for _, ln in ipairs(r.lines) do table.insert(final, ln) end
    cur = r.end_row + 1
  end
  while cur <= #orig_lines do
    table.insert(final, orig_lines[cur]); cur = cur + 1
  end
...
function M.format_blocks_async(bufnr, sline, eline, opts)
  ...
  local supported = {}
  for k, _ in pairs(M._config.formatters or {}) do supported[k] = true end
  local aliases = M._config.lang_aliases or {}

  -- find blocks in range
  local blocks = finder.find_blocks_in_range(bufnr, sline, eline, supported, aliases, { prefer_treesitter = M._config.prefer_treesitter })
  if not blocks or #blocks == 0 then
    notify("No supported fenced codeblocks in range.")
    if opts.on_exit then opts.on_exit(true, { changed = false }) end
    return
  end

  notify(string.format("Found %d supported fenced block(s).", #blocks))
  for _, b in ipairs(blocks) do debug(string.format("found block lang=%s start=%d end=%d", tostring(b.lang), b.start_row, b.end_row)) end
  ...
  for _, block in ipairs(blocks) do
    ...
    format_single_block(bufnr, block, fmt_cfg, function(err, formatted_text)
      -- schedule work on main loop
      vim.schedule(function()
        if err then
          table.insert(errors, { block = block, err = tostring(err) })
          debug(string.format("format_single_block error for %s %d..%d : %s", block.lang, block.start_row, block.end_row, tostring(err)))
        elseif not formatted_text then
          -- no output: treat as non-changed; still log
          debug(string.format("format_single_block returned no formatted output for %s %d..%d", block.lang, block.start_row, block.end_row))
        else
          -- normalize trailing newline removal (we store lines without final empty)
          if formatted_text:sub(-1) == "\n" then formatted_text = formatted_text:sub(1, -2) end
          local new_lines = {}
          for ln in formatted_text:gmatch("([^\n]*)\n?") do table.insert(new_lines, ln) end

          -- fetch original block lines for comparison (use original snapshot to avoid race)
          local orig_block_lines = {}
          for i = block.start_row, block.end_row do table.insert(orig_block_lines, orig_lines[i]) end

          local same = true
          if #new_lines ~= #orig_block_lines then same = false else
            for i = 1, #new_lines do if new_lines[i] ~= orig_block_lines[i] then same = false; break end end
          end

          if not same then
            table.insert(replacements, { start_row = block.start_row, end_row = block.end_row, lines = new_lines })
            debug(string.format("Queued replacement for %s block %d..%d (orig=%d -> new=%d)", block.lang, block.start_row, block.end_row, #orig_block_lines, #new_lines))
          else
            debug(string.format("No change for %s block %d..%d (formatter output identical)", block.lang, block.start_row, block.end_row))
          end
        end
        ...
        if pending == 0 then
          ...
          if #replacements == 0 then
            ...
            if opts.on_exit then opts.on_exit(true, { changed = false, errors = errors }) end
            return
          end
          ...
          if #errors > 0 then
            ...
            for _, e in ipairs(errors) do debug(string.format("Error block %d..%d lang=%s : %s", e.block.start_row or 0, e.block.end_row or 0, e.block.lang or "?", tostring(e.err))) end
          end
          if opts.on_exit then opts.on_exit(true, { changed = true, replacements = replacements, errors = errors }) end
        end
      end)
...
function M.format_range_async(sline, eline)
  ...
  if not sline or not eline then
    ...
    if mode == "v" or mode == "V" or mode == "\22" then
      sline = fn.line("'<"); eline = fn.line("'>")
    else
      sline = fn.line("."); eline = sline
    end
  end
