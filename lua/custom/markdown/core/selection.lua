...
function M.get_visual_range_single_line()
  local p1 = fn.getpos("'<"); local p2 = fn.getpos("'>")
  local srow = p1[2] - 1; local erow = p2[2] - 1
  local scol0 = p1[3] - 1; local ecol0 = p2[3] - 1
  if srow ~= erow then return nil, nil, nil end
  ...
  if ecol0 < scol0 then return nil, nil, nil end
