...
---@field activate fun(cfg: HighlightCfg, mode: string|nil): nil # Activate cursorline/column for active window with optional mode tinting
---@field deactivate fun(): nil # Deactivate cursorline/column for inactive windows (map to Normal)
---@field resolve_line_hl fun(mode: string, cfg: HighlightCfg): string # Resolve CursorLine HL group based on mode (returns group name)
...
---@field update fun(cfg: HighlightCfg, ev: table|nil): nil # Update tint based on ModeChanged event or current mode
---@field clear_cache fun(win: integer): nil # Clear cached mode for closed window (call on WinClosed)
---@field enable fun(cfg: HighlightCfg): nil # Install ModeChanged/BufWinEnter/WinClosed autocmds
...
---@field enable fun(cfg: HighlightCfg): nil # Enable yank/put flash based on config flags
...
---@field enable fun(cfg: HighlightCfg): nil # Install DiagnosticChanged/BufEnter autocmds
...
---@field enable fun(cfg: HighlightCfg): nil # Install TermOpen autocmd
...
---@field enable fun(cfg: HighlightCfg): nil # Install CursorMoved/InsertEnter/BufLeave/WinLeave autocmds
...
---@field refresh fun(cfg: HighlightCfg): nil # Update indent scope highlight for current viewport (clears + reapplies)
---@field refresh_current fun(): nil # Refresh using global config (wrapper for after_set integration)
---@field enable fun(cfg: HighlightCfg): nil # Install BufEnter/CursorMoved/WinScrolled autocmds
...
---@field enable fun(cfg: HighlightCfg): nil # Install or clear `gh` keymap based on feature state + gitsigns availability
