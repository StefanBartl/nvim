...
---
---@field breadcrumbs_ctx WKDOptions.HL_CFG.Breadcrumbs.Ctx
---
...
---
--- The full color/face palette used by all features (see wkdoptionsHighlightColors).
--- Changes:
...
---@field build fun(cfg: HighlightCfg, ctx_fn: fun():string|nil): string # Build winbar string (path + separator + context) with skip checks + ellipsization
---@field apply fun(cfg: HighlightCfg, ctx_fn: fun():string|nil): nil # Build and apply winbar to current window (scheduled to avoid mid-event modification)
...
---@field refresh_with_config fun(cfg: HighlightCfg): nil # Refresh winbar using provided config (applies to current window)
