...
local function _collect_js_owner_from_literal(n)
  ...
  for _ = 1, 20 do
    ...
    elseif t == "assignment_expression" then
      ...
      if left then
        owner = _txt(left):gsub("%s+", ""):gsub("%=.*", "")
      end
      break
    end
    local p = v:parent()
    if not p or p == v then
      break
    end
    v = p
  end
...
local function _collect_lua_owner_from_table(n)
  ...
  if not found_field or #keys == 0 then
    return owner
  end
...
local function _ctx_lsp_func()
  local bctx = cfg.breadcrumbs_ctx or {} ---@type wkdoptionsBreadcrumbsCtx
  if not bctx.prefer_lsp_function then
    return nil
  end
...
local function _ctx_ts_symbol()
  local bctx = cfg.breadcrumbs_ctx or {} ---@type wkdoptionsBreadcrumbsCtx
  if not bctx.use_treesitter_symbol then
    return nil
  end
...
local function _ctx_with_container(base_symbol)
  ...
  local bctx = cfg.breadcrumbs_ctx or {} ---@type wkdoptionsBreadcrumbsCtx
  if not bctx.use_container_chain then
    return base_symbol
  end
  ...
  while u and steps < 12 and #chain < maxd do
    ...
    if not parent or parent == u then
      break
    end
    u = parent
    steps = steps + 1
  ...
  if bctx.dedupe_containers then
    chain = _dedupe_consecutive(chain)
  end
...
local function _ctx_lang_extra()
  local bctx = cfg.breadcrumbs_ctx or {} ---@type wkdoptionsBreadcrumbsCtx
  if not bctx.use_lang_specific then
    return nil
  end
  local node = _node_at_cursor()
  if not node then
...
local function _ctx_word_fallback()
  ...
  if not (bctx and bctx.fallback_word_when_empty) then
    return nil
  end
