kurze Übersicht
• Ziel: robusteres Debug-Hilfsmodul mit sauberer API, ohne globale Leaks, mit EmmyLua-Annotationen, besserer Fehlerbehandlung und praxisnahen Anwendungen (Keymaps, User-Commands, Autocommands).
• Highlights: sichere Dumps (auch in Fast-Events), wahlweise Notification oder Scratch-Buffer, Extmark-/Modul-Speicherübersicht mit Filtern, Upvalue-Inspektion, bequeme Kommandos/Keymaps.
...
Anwendung in deiner bestehenden Struktur

1. Dateiablage
   `lua/utils/debug.lua` (wie oben).

2. Initialisierung (z. B. in `plugins/personal.lua` oder einem init-Block):

```lua
-- Require once and set up commands/keymaps/autocmds
require("utils.debug").setup({
  set_global_dd = true,        -- defines _G.dd(...)
  create_commands = true,      -- :DbgEval, :DbgExtmarks, :DbgModules
  create_keymaps = true,       -- <leader>d{e,x,m,v}
  create_autocmds = false,     -- set true if du die Warn-Autocmd willst
  leader = "<leader>d",        -- prefix for keymaps
  default_method = "notify",   -- or "float" to always use a scratch window
})
```

3. Typische Workflows

* Schnell dumpen (global):
  `:lua dd(vim.o)`
* Ausdruck auswerten:
  `:DbgEval vim.o.lines` oder `<leader>de` und Eingabeaufforderung nutzen
* Extmarks überblicken:
  `:DbgExtmarks` oder `<leader>dx`
  `:DbgExtmarks 500` (nur Einträge mit ≥500 Extmarks)
* Modulgrößen prüfen:
  `:DbgModules` oder `<leader>dm`
  `:DbgModules '^plugins%.' 20` (nur plugins.\*, Top 20)
* Visuelle Auswahl dumpen (ohne Eval):
  Markieren und `<leader>dv`

4. Verbesserungen gegenüber deiner Vorlage

* Keine globalen Funktions-Leaks (`estimate_size` ist lokal).
* Große Dumps wechseln automatisch auf ein Float-Fenster (verhindert Notify-Truncation).
* Einheitliche EmmyLua-Annotationen.
* Stabil in Fast-Events (via `vim.schedule`).
* Saubere Keymaps/Commands out of the box.
* Optionale Autocommand-Warnung bei großen Extmark-Zahlen.

Wenn gewünscht, ergänze ich noch eine Variante von `DbgEval` mit eigenem, minimalem Environment (Sandbox), um zufällige Seiteneffekte zu vermeiden – aktuell nutzt es `_G` bewusst als komfortables Dev-Tool.
