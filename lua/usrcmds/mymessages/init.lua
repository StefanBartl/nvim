...
local function detect_platform()
  ...
  if not uname then
    vim.notify("[usrcmds.mymessages] uname is nil")
    return nil
  end
...
local function copy_to_clipboard(text, debug)
  ...
  if ok then
    if debug then
      vim.notify("MyMessages: setreg('+') ok (system provider may still be required)", vim.log.levels.DEBUG)
    end
    return true
  end
  ...
  if not P then
    vim.notify("[usrcmds.mymessages] P is nil")
    return nil
  end
  ...
  if P.is_mac and has_exec("pbcopy") then
    ...
    if debug then
      vim.notify("pbcopy failed: " .. tostring(err), vim.log.levels.DEBUG)
    end
  end
  ...
  if P.is_wsl or P.is_win then
    if has_exec("clip.exe") then
      ...
      if debug then
        vim.notify("clip.exe failed: " .. tostring(err), vim.log.levels.DEBUG)
      end
    end
    ...
    if not has_exec("clip.exe") and vim.fn.filereadable(clip_abs) == 1 then
      ...
      if debug then
        vim.notify("abs clip.exe failed: " .. tostring(err), vim.log.levels.DEBUG)
      end
    end
  end
  ...
  if P.is_wayland and has_exec("wl-copy") then
    ...
    if debug then
      vim.notify("wl-copy failed: " .. tostring(err), vim.log.levels.DEBUG)
    end
  end
  ...
  if P.is_x11 then
    if has_exec("xclip") then
      ...
      if debug then
        vim.notify("xclip failed: " .. tostring(err), vim.log.levels.DEBUG)
      end
    end
    if has_exec("xsel") then
      ...
      if debug then
        vim.notify("xsel failed: " .. tostring(err), vim.log.levels.DEBUG)
      end
    end
  end
...
function M.run(opts)
  ...
  if debug then
    vim.notify(("MyMessages: captured %d bytes"):format(#messages), vim.log.levels.DEBUG)
  end
  ...
  if not ok_clip then
    vim.notify(
      "MyMessages: clipboard not available. Install a provider (pbcopy/wl-copy/xclip/xsel/clip.exe).",
      vim.log.levels.WARN
    )
  elseif debug then
...
function M.enable_usercmds()
  vim.api.nvim_create_user_command("MyMessages", function()
    M.run({ debug = false })
  end, { desc = "Capture :messages, save to file, copy to clipboard" })

  vim.api.nvim_create_user_command("MyMessagesDebug", function()
    M.run({ debug = true })
  end, { desc = "Capture :messages with debug notifications" })
end
