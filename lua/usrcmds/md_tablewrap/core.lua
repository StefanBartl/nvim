...
local function expand_proportionally(widths, remainder, weights, cap)
  ...
  for i = 1, n do
    widths[i] = widths[i] + (added[i] or 0)
  end
  return widths
end
...
local function is_separator_line(line)
  if not is_table_like(line) then
    return false
  end
...
local function parse_aligns(sep_line, ncols)
  ...
  if not sep_line or sep_line == "" then
    for _ = 1, ncols do
      aligns[#aligns + 1] = { left = true, right = false }
    end
    return aligns
  end
...
local function find_table_bounds(buf, lnum)
  ...
  if lnum < 1 or lnum > total then
    return false, nil, "cursor out of range"
  end
  local lines = api.nvim_buf_get_lines(buf, 0, -1, false)
  if not is_table_like(lines[lnum]) then
    return false, nil, "not on a table line"
  end

  local s = lnum
  while s > 1 and is_table_like(lines[s - 1]) do
    s = s - 1
  end
  local e = lnum
  while e < total and is_table_like(lines[e + 1]) do
    e = e + 1
  end
...
local function find_all_table_bounds(buf)
  ...
  while i <= total do
    if is_table_like(lines[i]) then
      local s = i
      while i + 1 <= total and is_table_like(lines[i + 1]) do
        i = i + 1
      end
      local e = i
      local first = lines[s]
...
local function parse_table_lines(lines)
  ...
  for i, L in ipairs(lines) do
    if is_separator_line(L) and not sep_idx then
      sep_idx = i
    end
  end
  local ncols = 0
...
local function compute_content_total(win, indent_col, ncols, pad, cfg)
  ...
  if available <= 0 then
    return false, nil, "no horizontal budget"
  end
  local pipes = ncols + 1
  local content_total = available - pipes - (2 * pad * ncols)
  if content_total < ncols then
    return false, nil, "not enough space for minimal columns"
  end
...
local function compute_plan(indent_col, parsed, cfg)
  ...
  if not ok or not content_total then
  ...
  if cfg.width_mode == "minflex" then
    ...
    if cap and cap >= 1 then
      ...
      if oversum > 0 then
        ...
        if any then
          widths = expand_proportionally(widths, oversum, reweights, cap)
        end
      end
    end
...
local function pad_cell(s, width, align, pad)
  ...
  if align.left and align.right then
    left_extra = math.floor(rest / 2)
    right_extra = rest - left_extra
  elseif align.right and not align.left then
    left_extra = rest
    right_extra = 0
  end
  return string.rep(" ", pad)
    .. string.rep(" ", left_extra)
    .. s
    .. string.rep(" ", right_extra)
    .. string.rep(" ", pad)
end
...
---@param ch string
---@return string
local function repn(n, ch)
  return string.rep(ch, math.max(0, n))
end
...
local function build_separator_row(aligns, widths)
  ...
  for i, w in ipairs(widths) do
    ...
    if right and not left then
      core = core:sub(1, #core - 1) .. ":"
    end
    if left and right then
      core = ":" .. core:sub(2, #core - 1) .. ":"
    end
    parts[#parts + 1] = " " .. core .. " "
  end
...
local function format_body_rows(rows, widths, aligns, pad)
  ...
  for _, row in ipairs(rows) do
    ...
    for c, s in ipairs(row) do
      ...
      if #wrapped_per_col[c] > maxh then
        maxh = #wrapped_per_col[c]
      end
    end
    for k = 1, maxh do
      local parts = {}
...
local function reformat_current(mode, cfg)
  ...
  if mode ~= "force" then
    ...
    if not oko then
      return false, "overflow check failed"
    end
...
local function reformat_all(mode, buf, cfg)
  ...
  for _, b in ipairs(list) do
    if mode == "force" then
      ...
      if not oko then
        return false, nil, "overflow check failed"
      end
      if overflow then
        local ok, err = reformat_table(buf, 0, b, cfg)
        if not ok then
