...
function M.attach(ctx)
  ctx = ctx or {}
  local filetypes = ctx.filetypes or { "javascript", "javascriptreact", "typescript", "typescriptreact", "vue", "svelte" }
  ...
  api.nvim_create_autocmd("BufWritePre", {
    ...
    callback = function(ev)
      if not ctx._enabled then return end
      local ft = api.nvim_buf_get_option(ev.buf, "filetype")
      local allowed = false
      for _, v in ipairs(filetypes) do if v == ft then allowed = true break end end
      if not allowed then return end

      local root = core(ev.buf)
      -- run only if either config exists
      if check.has_eslint(root) then eslint_fix.eslint_fix(ev.buf) end
      if check.has_prettier(root) then prettier_fmt.prettier_format(ev.buf) end
    end,
  })
end
