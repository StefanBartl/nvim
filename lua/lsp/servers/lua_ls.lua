...
local function build_library()
  -- stdconfig points to ~/.config/nvim (XDG on Linux/macOS)
  local stdconfig = vim.fn.stdpath("config")
  ---@type string[]
  local lib = vim.api.nvim_get_runtime_file("", true)  -- Neovim + plugins runtime
  -- Prepend our local @types if it exists
  local local_types = stdconfig .. "/lua/@types"
  if vim.uv.fs_stat(local_types) then
    table.insert(lib, 1, local_types)
  end
  return lib
end
