...
---@param parts string[]
---@return string
local function join(parts) return vim.fs.joinpath(unpack(parts)) end
...
---@return string|nil
local function startpath_from() return uv.cwd() end
...
local function strict_root()
  ...
  if lua_markers then return lua_markers end

  local stdconfig = vim.fn.stdpath("config")
  if is_subpath(start, stdconfig) then return stdconfig end
...
local function find_type_dirs(root, opts)
  ...
  local PRUNE = { [".git"]=true, [".hg"]=true, [".svn"]=true,
    ["node_modules"]=true, [".pnpm-store"]=true,
    [".venv"]=true, [".direnv"]=true, [".mypy_cache"]=true, [".cache"]=true, ["__pycache__"]=true,
    ["build"]=true, ["dist"]=true, ["target"]=true, ["zig-cache"]=true, ["zig-out"]=true,
  }
      ...
      if it then
        while true do
          ...
          if kind == "directory" then
            if PRUNE[name] then goto inner_continue end
            local child = norm(join({ node.path, name }))
            if name == "types" or name == "@types" then matches[#matches+1] = child end
...
local function build_library(root)
  ...
  if root and is_subpath(root, stdconfig) then
    ...
    if is_dir(cfg_atypes) then lib[#lib+1] = cfg_atypes end
    if is_dir(cfg_types)  then lib[#lib+1] = cfg_types  end
  end
  ...
  if root and is_dir(root) then
    ...
    for i = 1, #found do lib[#lib+1] = found[i] end
  end
...
function M.setup(shared, opts)
  ...
  local settings = {
    Lua = {
      ...
      workspace = {
        ...
        ignoreDir = {
          ".git", "**/node_modules",
          "**/.venv","**/.direnv","**/.cache","**/.mypy_cache",
          "**/build","**/dist","**/target","**/zig-cache","**/zig-out",
        },
        useGitIgnore = true,
        maxPreload = 1500,
        preloadFileSize = 200,
  ...
  if type(vim.lsp.config) == "table" then
    vim.lsp.config("lua_ls", {
      ...
      filetypes = { "lua" },
      -- keep strict function for parity with previous behavior
    ...
    if opts.enable ~= false then pcall(vim.lsp.enable, "lua_ls") end
  end
end

---@nodiscard
---@return string|nil
function M.debug_root() return strict_root() end

---@nodiscard
---@return string[]
function M.debug_library() return build_library(strict_root()) end
