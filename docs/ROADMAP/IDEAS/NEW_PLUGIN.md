...
| `lua/options.lua` | ~250 Zeilen, rein deklarativ: `vim.opt`/`vim.o`/`vim.wo`-Settings (Appearance, Clipboard, Indentation, Search, Folding, Performance, Shell/WSL) + zwei `require("lib.nvim.bindings.autocmd")`-Blöcke + `require("wkdoptions.ui.line_numbers")` + `require("wkdoptions.set_diff_profile.selector")` | Keine |
...
| `lib.nvim.bindings.autocmd` in `wkdoptions/init.lua`, `options.lua` (Diff-Reset, Markdown-Fold) | bereits verwendet ✅ |
...
| `lib.strings`, `lib.map`, `lib.lazy` (laut README "Prerequisites") | Pfade beim Umzug gegen aktuellen lib.nvim-Stand verifizieren (`lib.lua.strings`, `lib.nvim.bindings.keymap`, `lib.lua.lazy`) — die README ist vermutlich älter als die aktuelle lib.nvim-Modulstruktur |
