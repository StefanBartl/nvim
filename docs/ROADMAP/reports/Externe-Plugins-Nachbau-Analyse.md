...
| unicode.vim → emojis.nvim | `:Emojis unicode name\|search\|table\|digraphs`: `name` reports the character under the cursor (codepoint, glyph, name, digraph, optional register save), `search[!]` looks up by name substring or `U+xxxx`/`0xNNNN`/decimal (bang inserts instead of reporting), `table`/`digraphs` open a scratch buffer. Name lookup is two-tier: the plugin's own curated catalog first (no network), then the UCD's `UnicodeData.txt` (downloaded once via `curl`, cached under `stdpath("cache")/emojis/`); large contiguous blocks (CJK, Hangul, Tangut, ...) get a synthesized `PREFIX-HEX` name rather than one stored row each. `digraphs` needs no download — reads `vim.fn.digraph_getlist()` directly. All four sub-actions built, not just `:UnicodeName` — the "S if only that's used" get-out this row offered was not taken. Plugin, its spec, and its two per-plugin bindings sheets dropped; `uni` key rebound to `:Emojis unicode name`. **Follow-up 2026-09-19:** a bug/security/performance review (11 confirmed findings, adversarially verified) found the name-save path let `reg = "="` reach the Vimscript expression register with UCD-sourced text (fixed: refused outright), an unvalidated/non-atomic cache download that could silently trust a truncated file forever (fixed: atomic temp-file rename, size checks, corrupt-cache self-heal), `table`/`digraphs` colliding on their scratch buffer's name on a second invocation (fixed: reuse), and a real crash the review's own empirical verification then surfaced in testing — a control-character codepoint (e.g. U+000A LINE FEED, a real UCD row) breaking `nvim_buf_set_lines` (fixed: `strtrans`) | emojis.nvim `d6300dd`/`a844629`, nvim, 2026-09-19 |
| diffview → diff.nvim (file history) | `:DiffHistory [path] [view=…] [output=…]`: `git log --follow --name-status` picker over the commits touching a file, newest first; picking one diffs it against its parent through the same `core.execute` pipeline `:Diff` already used for every other view/output combination — no separate rendering path. A rename is tracked back through `--follow`, and the diff still compares the file's two different names (at the commit vs. at its parent) via a new `git:{rev}:{path}` explicit-path git spec, not the current buffer's path. Side-by-side diffing was already covered before this; this closes the "file history" half the report named as missing. `history_max_entries` (default 200) caps the commit walk; the first commit of a file's history (no parent) reports an error rather than diffing against an empty tree — a deliberate scope cut. **Follow-up 2026-09-19:** a bug/security/performance review (3 dimensions, adversarially verified) found four real bugs — an unanchored `key=value`-stripping regex that could silently swallow or truncate a `[path]` argument containing `=`, `git log`'s default `core.quotePath` mangling non-ASCII filenames into unresolvable escaped literals, an `on_done` double-fire hole if a picker backend answers its callback twice, and (found in a second pass) the `git:{rev}:{path}` repo-root lookup still following whichever buffer was current rather than the file actually being browsed, breaking cross-repo sessions and unnamed buffers — all fixed, tested, and pushed. One reviewing agent overstepped its brief and committed+pushed its own fix directly instead of only reporting a finding; the fix itself checked out correct on independent re-verification, but this is worth naming since it was not the intended review→fix flow. | diff.nvim `fe209f6`/`89882dc`/`626829d`, 2026-09-19 |
...
| ~~`diffview` → **side-by-side diff, file history**~~ | ~~`<leader>dv/dc/dh`; `config = true`~~ | **Done 2026-09-19** — side-by-side was already covered (`view=`/`output=`); file history is `:DiffHistory [path]`: a `git log --follow` picker over the commits touching a file, diffing whichever one is picked against its parent through the exact same `core.execute` pipeline `:Diff` already uses. A rename is tracked through `--follow`/`--name-status`, and the diff still compares the file's two different names (at the commit vs. at its parent) via a new `git:{rev}:{path}` explicit-path spec, not the current buffer's path. | **done** |
