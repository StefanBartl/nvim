# ai.nvim / loomAI — Erledigtes (aus dem Handover ausgelagert)

> Herkunft: `nvim/docs/ROADMAP/handovers/ai/ai.nvim_loomai.md`. Am 2026-09-21
> wurden alle fertigen Abschnitte und Punkte hierher verschoben, im Handover
> bleiben nur die offenen. Die Abschnitte stehen unverändert (Zeitkapsel,
> Datumsangaben und Commit-Hashes wie damals), nur die jeweils noch offenen
> Restpunkte wurden herausgeschnitten und stehen im Handover.

## Verifikation beim Ausmisten (2026-09-21)

Zwei Punkte, die im Log unten noch als offen stehen, waren in Wahrheit
längst erledigt (gegen die Repos gegengeprüft, nicht nur der Doku geglaubt):

- **Phase 9 / "Nächste Schritte" Punkt 3, `pdfport.nvim`-Migration:** erledigt —
  `E:\repos\pdfport.nvim` Commit `a49b8f3 refactor(backends): route claude and
  ollama through ai.nvim`, `backends/claude.lua` nutzt `ai.attachments`, plus
  ein `gemini`-Backend (`21e8d18`).
- **"Nächste Schritte" Punkt 12, 13 Funde aus dem `rules.nvim`-Einzeldurchgang:**
  erledigt — `rules-nvim-review-full.md` führt sie als **14 von 14 gefixt**
  (`LUA-16`, `XP-05`, `ERR-50`, `ERR-60`, `NEW-19/20`, `LUA-84`,
  `PERF-64/62`, `REL-03/NEW-11` (README-ToC), `ERR-05`, `ERR-06/PRIN-21`,
  `ERR-54`, `UI-61`, `LUA-54`, `PRIN-34`).
- Ebenfalls erledigt: der Backlog-Eintrag "explain" im Wkdbook-ROADMAP
  (Phase 7 Nebenbemerkung) — dort nicht mehr vorhanden. Die Design-Fragen aus
  Phase 9 (`context.diagnostics`-Format strukturiert, Modell frei) sind
...
2. ~~`rules.nvim`s `release`-Gate zeigt keine automatisierten kritischen
...
3. ~~Phase 9 (Follow-up, nicht blockierend): `pdfport.nvim`-Migration.~~
   **Erledigt** (s. Verifikation oben). `loomai`-Provider ebenfalls erledigt (s. o.).
...
12. 13 Funde aus dem vollständigen `rules.nvim`-Einzel-Durchgang (noch
    ...
    Funde"), **inzwischen alle 14 gefixt** (s. Verifikation oben). Priorisiert waren die zwei mit größter praktischer
    Relevanz: `LUA-16` (`vim.NIL` nach `vim.json.decode()`
