...
- [ ] **FEATURES-Doku vereinheitlichen — Teil erledigt, ein Fork offen.**
  - **Erledigt (2026-08-26):** `pickers.nvim` hat jetzt einen echten Katalog (7 Themen, 26 Features), der Changelog liegt als `docs/CHANGELOG.md` daneben. `color_my_ascii.nvim` ist entwirrt — sieben Feature-Handbücher lagen *im* Features-Ordner und wurden als 102 zusätzliche Features gelesen; sie liegen jetzt unter `docs/guides/`, der Ordner heißt nicht mehr `docs/features/` (Kleinschreibung, als einziges Repo), und `docs/FEATURES.md` (ein Commit-Log) heißt `FEATURE_LOG.md`. `replacer.nvim`: ein fehlendes Bullet-Set nachgetragen.
  - **Die alte Zahl war falsch.** Gemessen wird jetzt mit `documentation.core.features.resolve` selbst, nicht nach Augenschein. `reposcope.nvim` hatte nie ein Problem (24 Features, 51 Bullets, 0 ohne), `replacer.nvim` hatte genau eines.
  - **Offen, klein:** Features ohne Metadaten-Bullets — `documentation.nvim` **72 von 107**, `lib.nvim` 11/94, `mdview.nvim` 6/50, `runtime-analysis.nvim` 5/19, dazu je 1–3 in buffer-ctx, cascade, filetree, images, language, markdown, pdfport, sandbox. Laut Format gültig, aber der Features-Tab rendert sie ohne Metadaten. Reine Fleißarbeit.
  - **Offen, Entscheidung nötig: neun Repos sind für den Features-Tab unsichtbar.** `diff`, `emojis`, `fileops`, `github_stats`, `lsp`, `migrate`, `recommender`, `sessions`, `spotlight` haben je ein **einzelnes `docs/FEATURES.md`** (zusammen 147 `##`-Abschnitte, sauber geschrieben). `CANDIDATE_FOLDERS` in `documentation.nvim/lua/documentation/core/features.lua:54` ist aber `{ "docs/FEATURES", "docs/features" }` — nur Ordner. `resolve()` gibt für diese neun `nil` zurück, der Tab zeigt „keine Features". Zwei Wege:
    - **(a) Parser erweitert** — eine einzelne `docs/FEATURES.md` zusätzlich akzeptieren. Eine Änderung in einem Repo, schaltet alle neun frei; `FEATURES_FORMAT.md` sagt selbst „one `FEATURES.md` for a small plugin", ist an der Stelle aber mehrdeutig (der Codeblock daneben zeigt einen Ordner).
    - **(b) Neun Repos umgebaut** — je nach Größe in `docs/FEATURES/`-Themen aufteilen, wie bei `pickers.nvim` gemacht. Konsequenter, aber neunmal Arbeit, und für ein kleines Plugin wie `emojis.nvim` (14 Abschnitte) ein Ordner mit einer Datei darin.
    - Meine Empfehlung: **(a)**, und `FEATURES_FORMAT.md` an der Stelle eindeutig machen. (b) bleibt für die Repos möglich, die aus der Einzeldatei herauswachsen.
