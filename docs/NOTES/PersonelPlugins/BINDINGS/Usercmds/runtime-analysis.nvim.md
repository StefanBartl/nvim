...
| `:RA startup start` | Beobachtet die Hauptschleife auf Stalls und meldet nach 12s. Ein libuv-Timer misst seine *eigene* Verspätung, also wird eine Blockade erwischt, woher sie auch kommt — inklusive libuv-Callbacks, die `:profile` nicht sieht. |
| `:RA startup watch` / `:RA startup report` | Dieselbe Messung, weiterlaufend, bis die Zeitleiste abgefragt wird. Der Report legt Plugin-Loads (mit lazys Ladezeit **und** Ladegrund), `VimEnter`, `VeryLazy`, `LspAttach` und LSP-Progress auf eine Uhr. |
| `:RA startup probe` | Druckt und yankt die `--cmd`-Kommandozeile, die den Startup selbst misst — der Timer muss ticken, bevor die Config gesourct wird, was ein lazy geladenes Plugin für sich selbst nicht arrangieren kann. |
...
| `:RATelemetry flush [ns]` | jetzt schreiben und weiter aufzeichnen — jede Instanz oder nur eine. Der periodische Flush, `stop` und `VimLeavePre` schreiben ohnehin; das hier kauft Gewissheit zu einem gewählten Zeitpunkt, ohne den Lauf dafür beenden zu müssen |
| `:RATelemetry flamegraph [path]` | dieselben Startup-Daten als Flamegraph-SVG — Breite ist Gesamtzeit, Tiefe ist Require-Verschachtelung, eine Farbe je Modulwurzel. Wird von `images.nvim` im Terminal gezeichnet, wenn es installiert ist (dessen gecachter SVG→PNG-Pfad), sonst an den System-Opener gegeben. Ohne `path` landet sie im Wegwerf-Cache neben den anderen Reports — **M12** |
