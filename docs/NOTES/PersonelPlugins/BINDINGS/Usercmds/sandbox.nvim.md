...
identical route tree) with `container`/`image`/`wsl` sub-namespaces —
`:Container`/`:Image`/`:Wsl` were too generic and collided conceptually
with unrelated plugins (e.g. an image/png viewer claiming `:Image`).
Breaking change again, no compat aliases for the old verb names.

Source: `lua/sandbox/bindings/usrcmds/init.lua` (verb + custom types),
...
| Command | Args | Effect |
| --- | --- | --- |
| `:Sandbox container {sub}` (alias `:Sbx container {sub}`) | `list\|logs\|exec\|exec-once\|start\|stop\|kill\|remove\|prune\|inspect` | Container ops |
| `:Sandbox image {sub}` (alias `:Sbx image {sub}`) | `list\|pull\|remove\|prune` | Image ops |
| `:Sandbox wsl {sub}` (alias `:Sbx wsl {sub}`) | `list\|start\|stop\|exec` | WSL distro ops (only registered if `wsl.exe` reachable) |
...
instead of a `vim.notify` summary, e.g. `:Sandbox container start <id>
--buffer`.
