...
| `d`/`U`/`<leader>th` | Trash current node (or batch chooser for marked nodes) / undo last trash / show trash history | `confirm=true` default deliberately differs from copy_move/rename_batch's `false`, since trash is easier to mis-click. `<leader>th` shadows lsp.nvim's global inlay-hint toggle while the tree is focused — see [Collisions.md](./Collisions.md#cross-scope-shadowing); the config `noop`s it in the read-only `diagnostics`/`document_symbols` sources |
