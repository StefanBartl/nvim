...
| `d`/`U`/`<leader>th` | Trash current node (or batch chooser for marked nodes) / undo last trash / show trash history | `confirm=true` default deliberately differs from copy_move/rename_batch's `false`, since trash is easier to mis-click. Seit 2026-09-02 (filetree.nvim `71eaa54`) nur noch in der `filesystem`-Source gebunden: `lua/filetree/sources.lua` schraenkt das ganze `trash`-Feature darauf ein, weil ein Symbol-Outline oder eine Diagnostics-Liste keinen Knoten hat, den man in den Papierkorb legen koennte. In diesen Baeumen kommt darum lsp.nvims globaler Inlay-Hint-Toggle durch. Die zwei `noop`-Zeilen der Config sind damit weg — sie haben ihn vorher mitverschluckt und gegen filetrees Injection ohnehin verloren. Siehe [Collisions.md](./Collisions.md#cross-scope-shadowing) |
