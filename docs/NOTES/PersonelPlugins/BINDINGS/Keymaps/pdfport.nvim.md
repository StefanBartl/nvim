...
- **nvim-tree** (`integrations/nvim_tree.lua`) — the 4 actions above, each checking the node under cursor is a `.pdf` and dispatching to `picker.pick_and_open`/`pdfport.open` with a different `mode`. Only registered if `require("nvim-tree.api")` succeeds and the key isn't set to `false`.
