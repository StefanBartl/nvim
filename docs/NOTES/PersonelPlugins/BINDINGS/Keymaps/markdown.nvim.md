...
| delete_link_file | n | `DD` | Loescht die Zeile **und** die Datei, auf die ihr erster Link zeigt — nach einem `lib.nvim`-`ui.kit.confirm`, der den aufgeloesten absoluten Pfad zeigt und zaehlt, wie viele *andere* Links auf dieselbe Datei zeigen (`core/file_refs.lua`, rg-Prefilter, async). Auf jeder anderen Zeile schlichtes `dd`, Count inklusive: URL, `mailto:`, `#anchor`, Verzeichnis, oder Ziel nicht auf der Platte (das sagt es dazu). `#fragment` wird vor dem Aufloesen abgeschnitten. Ohne lib.nvim kein Dialog und darum auch keine Loeschung, nur `dd` + Warnung. **`DD` verdeckt das eingebaute `D` fuer 'timeoutlen' in Markdown-Buffern** — `keymaps.delete_link_file = false` haengt es ab (seit 2026-09-02) | soft dep: lib.nvim (sonst nur `dd`) |
...
- 2026-09-02 (3): neuer Keymap `DD` (id `delete_link_file`) — Zeile plus verlinkte Datei loeschen, mit Bestaetigungsdialog davor. Der erste Aufrufer von `core/file_refs.lua` ueberhaupt; dessen Moduldoc sagte bis dahin, markdown.nvim habe kein Delete-Feature. Zwei Dinge, die man vor dem Uebernehmen wissen sollte: `DD` schiebt sich vor das eingebaute `D` (Wartezeit 'timeoutlen', nur in Markdown-Buffern), und ohne lib.nvim faellt die Taste auf `dd` zurueck statt ungefragt zu loeschen.
