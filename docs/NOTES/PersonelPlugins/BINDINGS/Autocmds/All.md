# Autocmds — Index

All plugin autocmd cheatsheets in this folder.

- [buffer-ctx.nvim](buffer-ctx.md)
- [cascade.nvim](cascade.nvim.md)
- [color_my_ascii.nvim](color_my_ascii.nvim.md)
- [dap.nvim](dap.nvim.md)
- [debugging.nvim](debugging.nvim.md)
- [diff.nvim](diff.nvim.md)
- [emojis.nvim](emojis.nvim.md)
- [fileops.nvim](fileops.nvim.md)
- [filetree.nvim](filetree.nvim.md)
- [github_stats.nvim](github_stats.nvim.md)
- [gopath.nvim](gopath.nvim.md)
- [language.nvim](language.nvim.md)
- [lib.nvim](lib.nvim.md)
- [markdown.nvim](markdown.nvim.md)
- [mdview.nvim](mdview.nvim.md)
- [migrate.nvim](migrate.nvim.md)
- [nvim-cmdlog](nvim-cmdlog.md)
- [nvim-containers](nvim-containers.md)
- [open.nvim](open.nvim.md)
- [pdfport.nvim](pdfport.nvim.md)
- [pickers.nvim](pickers.nvim.md)
- [project-insight.nvim](project-insight.nvim.md)
- [recommender.nvim](recommender.nvim.md)
- [replacer.nvim](replacer.nvim.md)
- [reposcope.nvim](reposcope.nvim.md)
- [sessions.nvim](sessions.nvim.md)

See also, cross-plugin analyses: [by plugin](../autocmds-by-plugin.md), [by event](../autocmds-by-event.md), [by filetype/scope](../autocmds-by-filetype.md).
