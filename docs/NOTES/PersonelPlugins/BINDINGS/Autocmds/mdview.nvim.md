...
Sources: `lua/mdview/bindings/autocmds/init.lua` (`M.attach()`/`M.teardown()`), `bufenter.lua`, `buffer_switch.lua`, `live_push.lua`, `scroll_sync.lua`, `selection_sync.lua`, `vim_leave.lua`, `preview_tab_sync.lua`
...
| `buffer_switch.lua` | `BufEnter` | `defaults.ft_pattern` | Applies `browser.behavior` ("reuse"/"new_tab"/"manual") when switching markdown buffers during an active session; no-ops if `open_preview_tab` is set, **or while a document pin is up** (`:MDView pin`, 2026-09-03) |
...
| `selection_sync.lua` | `ModeChanged` (`*:[vV<C-v>]*`, `[vV<C-v>]*:*`), `CursorMoved` | mode patterns / `defaults.ft_pattern` | Sends the current visual selection (shape + lines + 1-based byte columns) over `/control` so the browser highlights the same text; throttled 60ms with a trailing send, cleared on leaving visual mode. Condition: `browser.selection_sync` (**default off** — toggled on while presenting), checked **per event** rather than at attach time so `:MDView selection on` reaches the *running* session |
...
## Document pinning gates four of these (2026-09-03)

`:MDView pin` (`lua/mdview/core/pin.lua`) freezes the preview on one document.
It is enforced at the **source**, not in the relay: `buffer_switch` returns
before doing anything at all, and `live_push.push_buffer_changes`,
`scroll_sync.send_current_position` and `selection_sync.send_current_selection`
each ask `pin.blocks(bufnr)` right after their `previewable.is()` check.

`blocks()` is true only when *both* hold: the buffer is not the pinned document
**and** its traffic would target the pinned tab's room (`target_key.resolve`).
So the pinned document keeps driving the preview normally, and under
`new_tab`/`manual` — where every document has a room of its own — a pin blocks
nothing rather than muting the whole session.
