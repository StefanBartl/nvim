...
| `selection_sync.lua` | `ModeChanged` (`*:[vV<C-v>]*`, `[vV<C-v>]*:*`), `CursorMoved` | mode patterns / `defaults.ft_pattern` | Sends the current visual selection (shape + lines + 1-based byte columns) over `/control` so the browser highlights the same text; throttled 60ms with a trailing send, cleared on leaving visual mode. Condition: `browser.selection_sync` (default on), checked **per event** so `:MDView selection on` reaches the running session |
