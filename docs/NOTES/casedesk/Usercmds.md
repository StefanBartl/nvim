...
| `:Case activity [nr]` | — | Paste the system clipboard (an Activity Stream) into a new auto-numbered `Research/NN_ActivityStream.md`. Takes **either** source format, auto-detected, no flag: ServiceNow's own copy-paste, or SAP Resolve's Tampermonkey-formatted Conversations export (EXTRACTION.md §13). Also auto-detects Priority (SLA.md §6A), `sap_component`, `versions.server`/`versions.commander` (EXTRACTION.md §8), and `last_reply_sent` (from a "Send to Customer" memo / a "Returned Case to Customer" entry, EXTRACTION.md §5, never regresses a newer manual stamp) straight into `.case.json` — no manual step. SAP Resolve exports carry no Priority at all, so for those (and only those) it asks once, if none is known yet |
...
| `:Case ki [nr]` | — | Build the AI-analysis prompt (role + policies + this case's activity stream, from the clipboard, one line of SLA urgency context — SLA.md §6E — PLUS a full "Ermittelte Fakten" block: priority/impact, SAP Component, Commander/Server versions, TBox-Build/Api-Core, custom DLLs, current SNOW state, SLA clocks, and a doc-link version tally — EXTRACTION.md §7) and copy it back to the clipboard, ready to paste into an AI chat. Also saved as `Research/NN_KiPrompt.md`. If `:Case ocr` has already read this case's screenshots, their text goes in too, under its own "maschinell gelesen" heading so the model can tell a parsed fact from a guessed one — read from the existing sidecars only, this never runs OCR itself |
...
| `:Case ocr [nr] [--force] [--lang=<code>]` | — | Read the text out of this case's screenshots with `tesseract` (via images.nvim's `images.ocr`) and write one `<image>.ocr.md` sidecar next to each image under `assets/`. That is what makes the text greppable: `:Cases grep` walks every `*.md` under a case recursively, sidecars included. Skips images whose sidecar is already current (mtime-compared, so a redacted or replaced screenshot IS re-read); `--force` re-reads everything. Sequential, one image at a time. The sidecars also feed `:Case ki`'s prompt as a clearly-marked "machine-read" block — but deliberately NOT `:Case similar`, whose TF-IDF weighting would give every recognition error maximum weight |
...
| `:Case insert [field] [nr]` | `field` `<Tab>`-completed: `case\|snow\|link\|title\|company\|name\|priority\|summary\|mail-subject\|asset\|asset-path`; prompts via `kit.select` (showing live values) if omitted | Insert that token at the cursor AND copy it to the clipboard, one action. `link` is the SNOW ticket URL (falls back to the plain id if `config.snow_url_format` isn't set), `mail-subject` is `[case] title`. `asset`/`asset-path` open a second picker over the case's `assets/` (same file list as `:Case attachments`): `asset` inserts a markdown link relative to the buffer you're writing in (`[Log.txt](../assets/Log.txt)`, spaces and parens percent-encoded), `asset-path` the absolute path for pasting into Teams, Explorer or a shell. From a buffer outside the case, `asset` also falls back to the absolute path — a relative link would resolve to nothing there. With a Visual range (`:'<,'>Case insert [field]`) replaces the selection instead of inserting at the cursor — handy on a `<CASE>` placeholder |
...
| `:Cases grep <pattern> [--re\|-r]` | — | Full-text search across every case's `.md` files, recursively — attachments themselves are binary and stay out, but the `.ocr.md` sidecars `:Case ocr` writes under `assets/` are `.md` and are searched, so screenshot text is findable here. Report via `kit.viewer`, capped at 500 hits |
