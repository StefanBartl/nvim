...
-- =========================================================================
-- TODO: AUDIT: BUG:
-- SICHERE IMPLEMENTIERUNG DER TREESITTER-RANGE-FUNKTIONEN (HOTFIX)
-- =========================================================================

--- Hilfsfunktion: Ermittelt die Range eines Nodes absolut sicher.
-- Verhindert den Absturz "attempt to call method 'range' (a nil value)",
-- indem geprüft wird, ob das Node-Objekt existiert und die range-Methode besitzt.
-- Falls nicht, wird eine sichere Standard-Range (0, 0, 0, 0) zurückgegeben.
---@param node TSNode|table|nil Das zu prüfende Node-Objekt
---@param use_bytes boolean|nil Bestimmt, ob Bytes zurückgegeben werden sollen
---@return integer, integer, integer, integer
local function safe_node_range(node, use_bytes)
  if not node or (type(node) ~= "userdata" and type(node) ~= "table") or type(node.range) ~= "function" then
    return 0, 0, 0, 0
  end
  return node:range(use_bytes)
end


--- Hilfsfunktion: Berechnet den Versatz (Offset) einer Node-Range.
-- Nutzt jetzt safe_node_range für maximale Stabilität.
---@param node TSNode
---@param source integer|string Puffer-ID oder String, aus dem der Node stammt
---@param offset Range4
---@return Range6
local function apply_range_offset(node, source, offset)
  ---@diagnostic disable-next-line: missing-fields LuaLS varargs bug
  local range = { safe_node_range(node) } ---@type Range4
  ...
  -- Falls dies eine ungültige Range erzeugt, überspringen wir es sicher
  return { safe_node_range(node, true) }
end

--- Ermittelt die Range eines |TSNode|s. Kann auch {source} und {metadata}
-- entgegennehmen, um die Range mit angewandten Direktiven zu berechnen.
---@param node TSNode
---@param source integer|string|nil Puffer-ID oder String, aus dem der Node stammt
...
--- Bestimmt, ob ein Node eine bestimmte Range einschließt.
-- Prüft den Node vorab ab, um Fehler beim Aufruf von node:range() zu vermeiden.
---@param node TSNode
---@param range table
---@return boolean -- True wenn der Node die Range enthält
function M.node_contains(node, range)
  -- Erlaubt auch ein Table, damit Nodes gemockt werden können
  ...
  local nrange = { safe_node_range(node) } --- @type Range4
