...
- Einstieg `C:\Users\bartl\.config\wezterm\wezterm.lua` -> laedt `$REPOS_DIR/Configs\Terminals\wezterm\init.lua` (`REPOS_DIR=E:\repos`). Dort **keine** Einstellung `enable_kitty_graphics` (Grep: nur `terminal_safety.lua:23 enable_kitty_keyboard = false`). Padding in Zellen wegen OSC-1337 (`config/experimental.lua:60-68`), `default_prog` via `config/powershell.lua`.
